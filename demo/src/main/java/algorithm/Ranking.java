...
public class Ranking {
			...
			for (Element link : links) {
				...
				if (isRelevantSite(secondLayerUrl)) {
					...
					try {
						...
						Document secondDoc = Jsoup.connect(secondLayerUrl).userAgent("Mozilla/5.0").timeout(1000).get();
	...
	private boolean isRelevantSite(String url) {
		return (url.contains(".gov") || url.contains(".edu") || url.contains(".org")) && !url.contains("dictionary")
				&& !url.contains("wikipedia") && !url.contains("translate") && !url.contains(".pdf")
				&& !url.contains("wiktionary") && !url.contains("dict.");
	}
